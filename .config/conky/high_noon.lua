...
function conky_widgets()
	...
	cr = cairo_create(cs)
	air_clock(cr, 515, 600, 390) -- options: xc, yc, size
	cairo_destroy(cr)

-- Middle rings
	
	cr = cairo_create(cs) 
	ring(cr, 'cpu', 'CPU0', 100, 0xFFFFFF, 0.3, 0x000000, 1.0, 592, 600, 235, 10, -89, -15)
	cairo_destroy(cr)

	cr = cairo_create(cs) 
	ring_ccw(cr, 'cpu', 'CPU1', 100, 0xFFFFFF, 0.3, 0x000000, 1.0, 592, 600, 235, 10, -91, -160)
	cairo_destroy(cr)

	cr = cairo_create(cs) 
	ring_ccw(cr, 'cpu', 'CPU2', 100, 0xFFFFFF, 0.3, 0x000000, 1.0, 428, 600, 235, 10, 89, 15)
	cairo_destroy(cr)

	cr = cairo_create(cs) 
	ring(cr, 'cpu', 'CPU3', 100, 0xFFFFFF, 0.3, 0x000000, 1.0, 428, 600, 235, 10, 91, 160) 
	cairo_destroy(cr)

-- Top rings

	cr = cairo_create(cs)
	ring(cr, 'fs_used_perc', '/home', 100, 0xFFFFFF, 0.5, 0x000000, 1.0, 468, 273, 115, 10, 91, 160) 
	cairo_destroy(cr)
...
-- 	cairo_destroy(cr)
	...
	cr = cairo_create(cs)
	ring_ccw(cr, 'fs_used_perc', '/', 100, 0xFFFFFF, 0.5, 0x000000, 1.0, 551, 273, 115, 10, -91, -160)
	cairo_destroy(cr)
...
--	cr = cairo_create(cs)
--	ring(cr, 'fs_used_perc', 'eth1', 100, 0xFFFFFF, 0.15, 0x000000, 1.0, 468, 927, 115, 10, 91, 155) 
--	cairo_destroy(cr)

	cr = cairo_create(cs)
	ring_ccw(cr, 'fs_used_perc', '/video', 100, 0xFFFFFF, 0.15, 0x000000, 1.0, 468, 927, 115, 10, 89, 20) 
	cairo_destroy(cr)

--	cr = cairo_create(cs)
--	ring_ccw(cr, 'fs_used_perc', '/', 100, 0xFFFFFF, 0.15, 0x000000, 1.0, 551, 927, 115, 10, -91, -155)
--	cairo_destroy(cr)

	cr = cairo_create(cs)
	ring(cr, 'fs_used_perc', '/data', 100, 0xFFFFFF, 0.15, 0x000000, 1.0, 551, 927, 115, 10, -89, -20) 
	cairo_destroy(cr)

--	cr = cairo_create(cs)
--	photo_album('/home/awross/Pictures/wallpapers', 100, 100, 320, 180, 10, 5)
--	cairo_destroy(cr)
end
